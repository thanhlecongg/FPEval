...
BASE_OUTPUT="/home/locpb4/research_llm_to_code/FPEval/output_fixed"
BASE_FIXED="/home/locpb4/research_llm_to_code/FPEval/output_fixed"

# SRC_MODEL="Qwen/Qwen3-Coder-30B-A3B-Instruct"
# SRC_MODEL="Qwen/Qwen2.5-Coder-3B-Instruct"
SRC_MODEL="Qwen/Qwen2.5-Coder-1.5B-Instruct-moe-finetuned-ver-1-all-peft-500k-merged-100k-finetuned"
DST_MODELS=(
...
for lang_dir in "$BASE_OUTPUT"/*/; do
    ...
    for dst_model in "${DST_MODELS[@]}"; do
        ...
        if $DRY_RUN; then
            ...
            echo "  Files to copy (100%)  : $total"
            echo "  Selected files:"
            # printf '    %s\n' "${selected[@]}"
            echo ""
        else
            mkdir -p "$dst_dir"
            echo "[$lang] -> $dst_model (0/$total)..."
            printf '%s\n' "${selected[@]}" | python3 "$PY_SCRIPT" "$src_dir" "$dst_dir" "$SRC_MODEL" "$dst_model" "$total"
